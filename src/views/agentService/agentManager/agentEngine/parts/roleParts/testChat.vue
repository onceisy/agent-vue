...
import { queryLlm, queryWithHistory } from "@/api/agent";
import { ref } from "vue";
...
// 发送问答消息
const sendMsg = async () => {
  if (!msgCon.value || actionLoading.value) {
  ...
  if (!sid.value) {
    ...
    sid.value = generateUUID();
  }
  messages.value.push({ type: "user", text: msgCon.value });
  sendMessage.value.push({ type: "user", text: msgCon.value });
  const params = {
    sid: sid.value,
  ...
  const res = await queryWithHistory(params);
  if (res.code == 200) {
    messages.value[messages.value.length - 1].msg_time = res.msg_time || "";
    // 发送消息的时间，取msg_time
    ...
    messages.value.push({
      ...
      debug: res.debug,
      log: res.log,
      msg_time: res.response_time || ""
    });
    // 机器人返回消息的时间，取response_time
    msgCon.value = "";
  } else {
...
const openDetail = async (row: any) => {
  ...
  dialogTableVisible.value = false;
  checkDocVisible.value = false;
  messages.value = []; // 每次打开弹窗清空消息列表
  sendMessage.value = []; // 每次打开弹窗清空消息列表
  sid.value = ""; // 会话id清空
  robotName.value = agentTemplateStore.getCurrentAgent.name || "";
  actionLoading.value = false;
};
const msgTypeMap = {
  4: "未启用",
...
<template>
  <div>
          ...
          <div class="chat-window">
            ...
            <div class="message-list">
              <div
                ...
                :key="index"
                :class="{
                  'robot-message': message.type === 'assistant',
              ...
              >
                <div class="message-content">
                  ...
                  <div class="avatar-img">
                    <img
                      :src="
                        message.type === 'assistant' ? robotAvatar : userAvatar
                      "
                      alt="avatar"
                      class="avatar"
                    />
                  ...
                  <div class="msg-block">
                    ...
                    <template
                      v-if="message.type == 'assistant' && message.source == 1"
                    >
            ...
            <div class="send-chat-box-wrap">
              <div class="send-chat-box">
                <el-input
                  ...
                  resize="none"
                  placeholder="请输入内容"
                  @keydown.enter="handleTextareaKeydown($event)"
                />
                <span class="send-btn" @click="sendMsg">
                  <span class="iconfont fasong" />
                </span>
              </div>
            </div>
          </div>
    ...
    <el-dialog v-model="dialogPromptVisible" append-to-body title="prompt调试">
      <div>
