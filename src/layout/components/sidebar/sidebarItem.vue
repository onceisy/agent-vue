...
<template>
  <span v-if="props.item.hideNav" style="display: none" />
  <!-- error下的页面业务逻辑有用到，route又是根据文件夹动态插入的，为了减少现有业务代码的改动，判断到时error的路由，直接隐藏 -->
  <el-menu-item
    v-else-if="
  ...
  >
    <template #title>
      ...
      >
        ...
        <span
          class="icon iconfont"
          :class="props.item.meta && toRaw(props.item.meta.icon)"
        />
      </div>
      <span v-if="layout === 'horizontal'">
